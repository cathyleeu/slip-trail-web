...
function InputElement({ className, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        'w-full px-4 py-3 bg-surface-raised border border-border text-fg rounded-xl focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent transition-all',
        className
      )}
      {...props}
    />
